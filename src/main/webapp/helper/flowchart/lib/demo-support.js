...
(function () {
    ...
    window.jsPlumbToolkitDemoSupport = {
        // return a random hierarchy that contains at least 10 nodes. The hierarchy also contains positioning
        ...
        randomHierarchy: function (maxDepth, maxBreadth, maxWidth, maxHeight) {
            ...
            var _do = function () {
                    var h = [ ], c = 0, pad = 80,
                        _node = function (depth) {
                            c++;
                            var id = "w" + c, name = "" + c,
                                childCount = Math.floor(Math.random() * maxBreadth),
            ...
            while ((_h = _do()).length < 10);

            var out = { nodes: [], edges: [] };
            for (var i = 0; i < _h.length; i++) {
                out.nodes.push(_h[i][0]);
                for (var j = 0; j < _h[i][1].length; j++) {
                    out.edges.push({source: _h[i][0].id, target: _h[i][1][j]});
                }
            }
